...
io_server.on('connection', function (client) {
	...
	console.log('socket.io server [',myPid,']: Data from client ID ',client.id,': [ photobooth-socket ] =>  [',data,']');
	...
	case 'in progress':
	    trigger_armed = false;
	    if (ListOfClientIDs.indexOf(client.id) < 0)
		ListOfClientIDs.push(client.id);	    
	    break;
	case 'start-picture':
	    trigger_armed = false;
	...
	if (pos != -1)
	{
	    ...
	    console.log('socket.io server [',myPid,']: Active client disconnected - ',client.id,' - removed from array pos ',pos,' - remaining ',ListOfClientIDs);
	    
	    if (! ListOfClientIDs.length)
	    {
		console.log('socket.io server [',myPid,']: No more active clients connected - removing lock and arming trigger' );
		trigger_armed = true;
		collageInProgress = false;
	    } 
...
if (pinNum >= 1 && pinNum <= 40)
{
	...
	if (rpio.read(pin))
	{
		...
		console.log('socket.io server [',myPid,']: Reset server state machine - Time since button press [ms] ',dTimeTrigger);
		trigger_armed = true;
		collageInProgress = false;
		return;
	    }
	    ...
	    {
		/* Picture */
		console.log('socket.io server [',myPid,']: Button released - Normal  Press - Time since button press [ms] ',dTimeTrigger);
	    ...
	    {
		/* Collage */
		console.log('socket.io server [',myPid,']: Button released - Long Press - Time since button press [ms] ',dTimeTrigger);
